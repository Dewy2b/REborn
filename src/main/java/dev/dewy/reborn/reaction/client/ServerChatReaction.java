...
import dev.dewy.reborn.util.MessageUtil;
...
public class ServerChatReaction implements IPacketReactor<ServerChatPacket> {
    ...
    public boolean takeAction(ServerChatPacket packet) {
        ...
        REborn.LOGGER.log("(CHAT) " + pckMsg.getFullText());
        REborn.INSTANCE.sendToChildren(new ServerChatPacket(MessageUtil.fromJson(pckMsg.toJson()), packet.getType()));
        return false;
    }
