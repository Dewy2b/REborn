...
import dev.dewy.reborn.api.exception.REbornPluginConfigurationException;
import dev.dewy.reborn.util.YML;
...
public class Config {
    ...
    @ConfigSetting
    public String motd = "&aREborn &f" + REborn.VERSION;
        ...
        try {
            ...
            for (Field declaredField : this.getClass().getDeclaredFields()) {
                ...
                if (!yml.exists(target)) {
                    ...
                    REborn.LOGGER.log("Created " + target);
                    continue;
                }
                ...
                REborn.LOGGER.logDebug("Set " + target);
            }
            yml.save();
        } catch (IllegalAccessException ex) {
            REbornPluginConfigurationException exc = new REbornPluginConfigurationException("Configuration error while reading " + this.getClass().getSimpleName());
    ...
    protected final void save() {
        try {
            ...
            for (Field declaredField : this.getClass().getDeclaredFields()) {
                ...
                REborn.LOGGER.logDebug("Saved " + target);
            }
            yml.save();
        } catch (IllegalAccessException ex) {
            REbornPluginConfigurationException exc = new REbornPluginConfigurationException("Configuration error while writing " + this.getClass().getSimpleName());
            exc.setStackTrace(ex.getStackTrace());
            throw exc;
        }
    }
