package dev.dewy.reborn.client;
 ...
 */
public class REbornClient implements SessionListener {
    ...
    public void packetReceived(PacketReceivedEvent ev) {
        ...
        try {
            if (!reactionRegistry.containsKey(ev.getPacket().getClass())) { // so we aren't blocking packets that dont need special processing
                REborn.INSTANCE.sendToChildren(event.getRecievedPacket());
                return;
            }
            this.reactionRegistry.forEach((pck, reactor) -> { // iterate over the registered reactions
                if (pck == ev.getPacket().getClass()) { // if the reaction is paired with pck's clas
                    ...
                    if (flag) // perform the action
                        REborn.INSTANCE.sendToChildren(event.getRecievedPacket()); // send the packet to children if true
                } //ez
            });
        } catch (Exception e) {
    ...
    public void disconnected(DisconnectedEvent disconnectedEvent) {
        REborn.LOGGER.logWarning("Disconnected: " + disconnectedEvent.getReason());
     ...
     */
    public static class REbornClientCache {

        public static REbornClientCache INSTANCE;
