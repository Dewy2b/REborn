...
public class REbornPluginLoader {
    ...
    private static final List<REbornPlugin> pluginList = new ArrayList<>();

    public REbornPluginLoader() {
    ...
    public void loadPlugins() {
        ...
        theRawPlugins.forEach((jar, info) -> {
            try {
                ...
                if (clazz.getSuperclass() == null || clazz.getSuperclass() != REbornPlugin.class) {
                    //invalid
                    LOGGER.logError(info.pluginName + " has an invalid main class, cannot load.");
                    return;
                }
                REbornPlugin plugin = (REbornPlugin) clazz.newInstance();
    ...
    public static List<REbornPlugin> getPluginList() {
        return pluginList;
    }
}
